      ...
      <ContextMenuTrigger>
        <div class="overflow-hidden card rounded-md relative">
          <img v-if="file.cover"
            :src="file.cover"
            :alt="file.name"
            class="card-image h-auto w-auto object-cover aspect-square"
          />
          <div v-else
            class="card-image--no-cover overflow-hidden rounded-xl border bg-card text-card-foreground shadow p-4 relative"
            >
            <svg
              height="100%"
              fill="#FAFAFA"
          ...
          <div class="card-action p-10" :class="{ 'is-playing': isCurrentTrackPlaying() }">
            <div class="track" :class="{'playing': isCurrentTrackPlaying()}">
              <div class="card-action__cover">
                <button class="play" @click="playHandler(file)"></button>
